...
from utils import lang_list, country_alpha3, top_country_list, french_top_country_list
...
if __name__ == '__main__':
    ...
    parser.add_argument("-dt", "--data", dest="data",
                        default="disaster_all_lang", type=str,
                        help="can be chosen from disaster_en, disaster_es, disaster_fr, disaster_all_lang, conflict.")
    parser.add_argument("-opt", "--option", dest="option",
                            default="rm_pair_country_summary", type=str,
                            help="can be chosen from gm_country, rm_pair_country, rm_pair_country_summary, rm_pair, rm_country.")
    ...
    elif args.data == "conflict":
        source_route = "data/conflict_news_count/"
    ...
    if args.option == "gm_country":
        ...
        # top_country_list = ['USA', "GBR", "CAN", "AUS", "MEX", "COL", "ESP", "ARG", "COD", "FRA"]

        select_country = 'USA'

        cur_data = pd.read_csv(output_route)
        cur_data = cur_data[cur_data['report_country'] == select_country]
    ...
    if args.option == "rm_pair_country_summary":
        ...
        for country_idx in range(len(country_pair_diplomatic_relation_dict["Destination"])):
            try:
                ...
                pass
        # investment flow
        country_pair_investment = pd.read_csv(
            data_route + "country_info/economy_flow_raw_data/country_investment_flow.csv")
        country_pair_investment = country_pair_investment[country_pair_investment["INDICATOR"] == "VALUE"]
        country_pair_investment = country_pair_investment[["REPORT_CTRY", "PARTNER_CTRY", "2018"]]
        ...
        for cur_lang in ["en","es","fr"]:
            ...
            model_sets_res = summary_col(model_sets, model_names = cur_lang_countries,stars=True)

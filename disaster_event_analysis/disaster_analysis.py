...
if __name__ == '__main__':
    ...
    parser.add_argument("-opt", "--option", dest="option",
                            default="rm_country", type=str,
                            help="can be chosen from rm_pair, rm_country.")
    args = parser.parse_args()

    # twitter_sample = pd.read_json('/Users/xichen/Downloads/1706468600801360379.json', lines=True)

    with open('/Users/xichen/Downloads/1706468600801360379.json') as json_data:
        d = json.load(json_data)

    data_route = "../ner_art_sampling/"
    ...
    if args.option == "rm_pair":
        ...
        for i in range(len(country_syn_list)):
            for j in range(len(country_syn_list)):
                if i == j:
                    ...
                    if (cur_alpha3_1 not in country_disaster_pairwise_count) or (cur_alpha3_2 not in country_disaster_pairwise_count[cur_alpha3_1]):
                        continue

                    country_pair_syn_dict_dict_dict[cur_alpha3_1][cur_alpha3_2]["disaster_art_count"] = country_disaster_pairwise_count[cur_alpha3_1][cur_alpha3_2]
        ...
        for search_metric in ['vif','aic']:
            ...
            inter_country_train_data_df.columns = ['disaster_art_count', "Neighbor", "Geography distance","Continent sim", "Region1 sim",\
            ...
            X = inter_country_train_data_df.drop(['disaster_art_count'], axis=1)
    ...
    if args.option == "rm_country":
        ...
        country_religion_distribution_list = pd.merge(country_migration_list, country_religion_distribution_list,
                                                      how='left', on='Country')
        ...
        country_pop_area_list = pd.merge(country_religion_distribution_list, country_pop_area_list, how='left',
                                         on='Alpha-3 code')
        ...
        for i in range(len(country_syn_list)):
            for j in range(len(country_syn_list)):
                if i == j:
                    ...
                    if cur_alpha3_1 not in country_disaster_count:
                        continue

                    country_pair_syn_dict_dict_dict[cur_alpha3_1][cur_alpha3_2]["disaster_art_count"] = country_disaster_count[cur_alpha3_1]
        ...
        for search_metric in ['vif', 'aic']:
            ...
            for cur_alpha3_1 in country_pair_syn_dict_dict_dict:
                for cur_alpha3_2 in country_pair_syn_dict_dict_dict[cur_alpha3_1]:
                    cur_country_pair = copy.deepcopy(country_pair_syn_dict_dict_dict[cur_alpha3_1][cur_alpha3_2])
                    cur_country_pair_sim = cur_country_pair["disaster_art_count"]
            ...
            intra_country_train_data_df.columns = ['disaster_art_count',  "Number of language",
                                                   ...
                                                   "Religion Disstribution Entropy", "Population", "Area"]

            # 0 is the pair number, 1 is the average similarity
            y = intra_country_train_label
            X = intra_country_train_data_df.drop(['disaster_art_count'], axis=1)

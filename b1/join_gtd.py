...
kb = json.load(open("gtd-disaster-instance-info.json"))
...
for ff in efiles:
    ...
    for dd in (json.loads(line) for line in open(ff)):
        ...
        for k in ['num fatalities','num injured']:
            ...
            k2 = k.replace(" ","_").replace("num","n").replace("affected","aff").replace("total","tot").replace("homeless","hl").replace("injured","inj").replace("deaths","de").replace("fatalities","fatal")
            dd[k2] = val

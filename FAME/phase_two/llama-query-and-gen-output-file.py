...
for param_count in ['8','70']:
    for dis_inst in evaldisasternames:
        arr_of_texts = []
        if "attack" not in dis_inst:
            f = open('9.23exps-fr-nd/'+dis_inst+'-filteredtext.json')
        else:
            f = open('gtd/fr/'+dis_inst+'-filteredtext.json')
        data = json.load(f)
        for t in data["arr"]:
        ...
        for t in arr_of_texts: 
            ...
            for event in replicate.stream(
                "meta/meta-llama-3-"+param_count+"b-instruct",
                input=input
            ):
                generated_text = str(event)
                if "yes" in generated_text.lower():
                    val = 1
                    seen_and_yes.append(t[0])
                    final.append(t)
                else:
                    val = 0
                    seen_and_no.append(t[0])
            i = i +1

        with open('passing-phase-2-fr-llama/'+param_count+'b/'+dis_inst+'-finalout.jsonl', 'w') as outfile:
            for entry in final:
